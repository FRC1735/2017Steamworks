...
public class Turret extends Subsystem {
    ...
    private final Solenoid cameraLightRelay = RobotMap.turretCameraLightRelay;
    ...
    public void run()        { operate(false); } // Arg is whether to init the subsystem
    public void initialize() { operate(true);  } // Arg is whether to init the subsystem
    private void operate(boolean initialize) {
    	double targetAngle;
    	double currentSetpoint = turretTurner.getSetpoint();
    	// Are we actively targeting? And have we reached the last snapshot's target position or are intializing the sytem?  if so, calculate a new target
    	if (this.isVisionEnabled() && (this.onTarget() || initialize)) {
    ...
    public boolean onTarget() {
    	// Is Error reported in (Fractional) rotations, or in encoder ticks (with one rotation being 1024 units)?
    	// It sounds like it returns raw units of encoder ticks.
    	// Therefore, if we want error of 1%, we want the reported error to be <= 10.24
    	double currentError = Math.abs(turretTurner.getClosedLoopError());
    	Robot.dbgPrintln("Current turret error is: " + currentError + "encoder ticks");
    	if (currentError <= 10.24) {
    		return true;
    	}
    	else {
    		return false;
